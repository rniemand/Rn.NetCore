...
public class GetListTests
{
  ...
  public async Task GetList_GivenCalled_ShouldSubmitMetric()
  {
    ...
    connectionHelper
      ...
      .Returns(new List<UserEntity> { new(), new() });
    ...
    Assert.That(builtMetric.Tags["repo_method"], Is.EqualTo("GetUsers"));
    Assert.That(builtMetric.Tags["command_type"], Is.EqualTo("GetList"));
    ...
    Assert.That(builtMetric.Fields["results_count"], Is.EqualTo(2));
  ...
  [Test]
  public async Task GetList_GivenExceptionThrown_ShouldReturnEmptyCollection()
  {
    // arrange
    var connectionHelper = Substitute.For<IDbConnectionHelper>();
    var ex = new Exception("Something bad");
    ...
    // act
    var entries = await testRepo.GetUsers();

    // assert
    Assert.That(entries, Is.InstanceOf<List<UserEntity>>());
  ...
  [Test]
  public async Task GetList_GivenResultsReturned_ShouldReturnResults()
  {
    // arrange
    var connectionHelper = Substitute.For<IDbConnectionHelper>();
    var logger = Substitute.For<ILoggerAdapter<TestRepo>>();
    ...
    // act
    var results = await testRepo.GetUsers();

    // assert
    Assert.That(results, Is.InstanceOf<List<UserEntity>>());
    Assert.That(results.First(), Is.EqualTo(userEntity));
